...
function check_requirement() {
  ...
  if (!fs.existsSync(config.gtps_folder + config.player_folder)) throw new Error("Player folder was not found at: " + config.gtps_folder + config.player_folder);
  else if (!fs.existsSync(config.gtps_folder + config.world_folder)) throw new Error("World folder was not found at: " + config.gtps_folder + config.world_folder );
  else if (!fs.existsSync(config.gtps_folder)) throw new Error("GTPS folder was not found at: " + config.gtps_folder);
  info_log("Configuring");
  let list = Object.keys(config).filter((fil) => fil !== "user_id" && fil !== "delay");
  for (let l of list) {
    if (!config.role_id && config.user_id) return;
    if (!config[l]) throw new Error(`Please set "${l}" in config.json`);
  }
  ...
  if (config.delay && ms(config.delay) < ms("5m")) throw new Error(`Delay must be a minimum of 10 minutes`);
  info_log("Requirements checked");
}
...
function check_using_http() {
  if (fs.existsSync("./"+config.gtpsname+".zip")) {
    let [size, name] = convertBytes(fs.statSync("./"+config.gtpsname+".zip").size).split(" ");
...
function send_backup({ http = null, msg = null, client = null }) {
  let dsc = stripIndent(`
  Worlds: ${getAllFiles(config.gtps_folder + config.world_folder).length}
  Players: ${getAllFiles(config.gtps_folder + config.player_folder).length}
  `);
  ...
  save_data(Date.now());
  zip(config.gtps_folder, ""+config.gtpsname+".zip", (err) => {
    if (err) return console.error(err);

    let opt = { embed, files: [{ attachment: "./"+config.gtpsname+".zip", name: "Backup-Result.zip" }] };
    opt = check_using_http() ? { embed } : opt;

    let ifManual = msg
      ? msg.author.send(opt).then((m) => msg.channel.send("Backup is in your DMs"))
      : client.channels.cache.get(config.secret_channels).send(opt);

    if (msg) msg.channel.send("Uploading & sending backup file");
    if (check_using_http()) {
      ...
      embed.addField(
        "Backup Link",
        `[Download Link](http://${data.ip}:7119/"+config.gtpsname+".zip?keydw=${data.key})`,
        true
      );
      if (config.delay) embed.addField("Expiry", "```" + config.delay + "```", true);
